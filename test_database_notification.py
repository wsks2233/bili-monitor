...
def test_email_notification(dynamic):
    """测试邮件通知"""
    print("\n测试邮件通知...")
    ...
    # 发送通知
    result = email_notifier.send(dynamic)
    if result.success:
        print(f"✅ 邮件通知发送成功：{result.message}")
    else:
        print(f"❌ 邮件通知发送失败：{result.message}")
...
def main():
    ...
    # 测试邮件通知
    test_email_notification(dynamic)
